...
function bigbluebuttonbn_get_recording_data_row($bbbsession, $recording,
    ...
    // Set recording_types.
    //Manju: commented to remove playback column.
    ...
    //Manju: commented to remove description column. 31/08/2020.
    ...
    //     //Manju: Tool bar icons are coming here.
    //     $rowdata->actionbar = bigbluebuttonbn_get_recording_data_row_types($recording, $bbbsession);
    // }
	$rowdata->actionbar = bigbluebuttonbn_get_recording_data_row_types($recording, $bbbsession);
...
function bigbluebuttonbn_get_recording_data_row_actionbar($recording, $tools) {
    ...
    //Manju: commented to add download and presentation icons to toolbar.
...
function bigbluebuttonbn_get_recording_data_row_types($recording, $bbbsession) {
	...
	$id = 'playbacks-'.$recording['recordID'];
            //Getting the meeting serial id
	$recordingtypes = html_writer::start_tag('div', array('data-imported' => $dataimported,
		'data-meetingid' => $recording['meetingID'],
		'data-recordingid' => $recording['recordID'],
		'title' => $title));

	foreach ($recording['playbacks'] as $playback) {
		$exploaddata = explode("-", $recording['meetingID']);
		$finalvideolink='';
		...
		if(!empty($exploaddata)){
			...
			if($handle1){
				$publishclass =' style="color:green !important;padding:5px;"';
			}else{
				$publishclass =' style="color:orange !important;pointer-events: none !important;padding:5px;"';
			}
			...
			if($studentvideo == 1){
				$studentdownloadvideo = '<a href="'.$videodownlink.'" class="action-icon btn-action text-truncate" '.$publishclass.' target="_blank"><i class="fa fa-download" aria-hidden="true"></i></a>';
			}
		}
        //Manju:here getting the presentation button.
		$recordingtypes .='<span id='.$id.' '.$visibility.' >';
		$recordingtypes .= bigbluebuttonbn_get_recording_data_row_type($recording, $bbbsession, $playback);
                //Manju: checking for admin or teacher.

		$roleid = $DB->get_field('role', 'id',array('shortname'=>'editingteacher'));

	//	$isteacheranywhere = $DB->record_exists('role_assignments', array('userid' => $USER->id, 'roleid' => $roleid));
		...
		if ($isteacheranywhere || is_siteadmin($USER->id)) {
			...
			$recordingtypes .= '&nbsp;&nbsp;<a href="'.$attendancehref.'" class="action-icon btn-action text-truncate" target="_blank"><i class="fa fa-users" aria-hidden="true"></i></a>&nbsp;&nbsp;';
			...
			$recordingtypes.= '&nbsp;&nbsp;&nbsp;&nbsp;<a href="'.$downloadhref.'" class="action-icon btn-action text-truncate" '.$publishclass.' target="_blank"><i class="fa fa-file-archive-o" aria-hidden="true"></i></a>';
...
function bigbluebuttonbn_get_recording_data_row_type($recording, $bbbsession, $playback) {
	...
	$text = '<i class="fa fa-television" aria-hidden="true"></i>';
    //Manju:adding for download link.
	...
	$linkattributes = array(
		...
		'class' => 'action-icon btn-action text-truncate',
...
function bigbluebuttonbn_get_recording_table_row($bbbsession, $recording, $rowdata) {
    //Manju: inserting records into publish table.
	global $DB;
    //Manju: inserting to publish table.
	$modid = $bbbsession['cm']->id;
	$meetingid = $recording['recordID'];
	$publish = 0;
	$publishtime = $recording['startTime'];
	$insert  = new stdClass();
	$insert->cmid = $modid;
	$insert->meetingid = $meetingid;
	$insert->publishflag = $publish;
	$insert->plublishdate = $publishtime;
	$insert->filesize = '';
	bigbluebutton_publish_insert($insert);
	...
	$row->cells = array();
    //Manju: Commented to remove playback button.
...
//-----------
function bigbluebuttonbn_settings_extra(&$renderer){
        // Configuration for 'notify users when recording ready' feature.
	if ((boolean)\mod_bigbluebuttonbn\settings\validator::section_settings_extended_shown()) {
		$renderer->render_group_header('extrasettings');
		$renderer->render_filemanager_default_file_picker('default_background');
		$renderer->render_filemanager_upload_a_pre_roll('upload_preroll');
        //Manju: Added for publish url. 14/09/2020.
		$renderer->render_group_element_text_publish('publish_url');
	}
}
//-----------
