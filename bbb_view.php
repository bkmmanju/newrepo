...
switch (strtolower($action)) {
    ...
    if (bigbluebuttonbn_is_meeting_running($bbbsession['meetingid'])) {
        ...
        //Manju: create a user record on joining the meeting.24/09/2020.
        $insert = new stdClass();
        $insert->course = $bbbsession['course']->id;
        $insert->bigbluebuttonbnid = $bbbsession['bigbluebuttonbn']->id;
        $insert->userid = $bbbsession['userID'];
        $insert->jointime = time();
        $insert->lefttime = '';
        $insert->recordingid = '';
        $insert->meetingid = $bbbsession['meetingid'];
        $insert->role = '';
        $DB->insert_record("bigbluebutton_attendance", $insert);
